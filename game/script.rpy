...
label apotheke:
    "Du betrittst die Apotheke. Hinter dem Verkaufstresen steht ein Apotheker und sortiert einige Packungen ein."

    apo "Guten Tag. Willkommen in der Apotheke."
    apo "Wie kann ich Ihnen helfen?"

    menu:
        "Nach einem Schwangerschaftstest fragen":
            m "Ich brauche einen Schwangerschaftstest."
            apo "Natürlich. Wir haben verschiedene Tests vorrätig."
            apo "Die Anwendung ist normalerweise unkompliziert. Befolgen Sie einfach die Anleitung in der Packung."
            apo "Ein Test kostet 25 Dollar."

            menu:
                "Einen Schwangerschaftstest kaufen (-25$)" if geld >= 25:
                    $ geld -= 25
                    $ inventar.append("Schwangerschaftstest")
                    apo "Bitte schön. Bewahren Sie die Packung bis zur Anwendung trocken auf."
                    m "Danke."
                    jump wohnheim_flur
        ...
        "Nach der Pille danach fragen":
            m "Haben Sie die Pille danach?"
            apo "Ja. Wenn Sie sie benötigen, sollten Sie möglichst schnell handeln."
            apo "Ich kann Ihnen das Präparat für 40 Dollar geben."

            menu:
                "Die Pille danach kaufen (-40$)" if geld >= 40:
                    $ geld -= 40
                    $ inventar.append("Pille danach")
                    apo "Bitte beachten Sie die Packungsbeilage und die dort angegebenen Hinweise."
                    m "Alles klar. Danke für die Beratung."
                    jump wohnheim_flur

                "Zurück":
                    jump apotheke

        "Nach Medikamenten und allgemeiner Beratung fragen":
            m "Ich wollte mich einfach einmal informieren."
            apo "Kein Problem. Wenn Sie Fragen zu Medikamenten haben, können Sie gerne nachfragen."
            apo "Bei Beschwerden, Unsicherheiten oder möglichen Nebenwirkungen sollten Sie sich entsprechend beraten lassen."
            m "Gut zu wissen."
            apo "Gibt es sonst noch etwas?"

            menu:
                "Nein, danke":
                    apo "Gerne. Einen schönen Tag noch."
                    jump wohnheim_flur
